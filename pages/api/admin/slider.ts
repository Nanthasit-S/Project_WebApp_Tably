...
const uploadEvent = async (req: NextApiRequest) => {
  ...
  if (imageFile) {
    ...
    const fileName = `${Date.now()}-${path.basename(new URL(imageUrlToFetch).pathname)}`;
    const newPath = path.join(uploadDir, fileName);
...
export default async function handler(
  ...
  if (req.method === "DELETE") {
    ...
    if (
      ...
      res.status(400).json({ message: "Missing ID or image URL" });

      return;
    }

    try {
      const result = await deleteEvent(idNumber, imageUrl);
