  ...
  try {
    const results = await withTransaction(async (conn) => {
      const completedResult = (await conn.query(
        "UPDATE bookings SET status = 'completed' WHERE booking_date < CURDATE() AND status = 'confirmed'",
      )) as UpdateResult;
      const deletedResult = (await conn.query(
        "DELETE FROM bookings WHERE booking_date < CURDATE() AND status = 'awaiting_confirmation'",
      )) as UpdateResult;
